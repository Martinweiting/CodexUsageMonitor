...
Codex Usage Monitor 是一個原生 Windows 桌面小工具，提供兩類資訊：

- Codex 帳戶的遠端額度與重置狀態；
- 從 Codex 工作階段紀錄產生的本機唯讀活動統計。

程式使用 Win32、Direct2D、DirectWrite、WinHTTP 與 C++20 實作，支援鍵盤/滑鼠操作，以及在複數螢幕之間移動。
...
透明度控制範圍是 20% 至 80%；數值越高，玻璃越透明。目前的繪圖方式刻意避免原生矩形背景，確保圓角小工具外的透明像素不會出現透明外框。
...
- **小工具太不透明或太透明：** 開啟選單中的「透明度」設定；支援範圍為 20–80%。
- **圖示或字型遺失：** 確認 `assets` 目錄位於執行檔旁。

## License

目前 repository 沒有附帶 license 檔案；重新散布程式前，請先確認預定的授權條款。
